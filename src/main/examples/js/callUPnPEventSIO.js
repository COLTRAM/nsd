...
require(["NSDPlusPlusSIO", "when", "jQuery", "bootstrap"], function (NSDPlusPlus, when, $, _) {
    ...
    function onserviceavailable() {
        NSDPlusPlus.logger("onserviceavailable callback "+counter++);
    ...
    function cb(progress) {
        document.getElementById("eventDisplay").textContent = progress + "";
    }

    window.subscribe = function() {
        service.subscribe("progress", cb);
    };

    window.unsubscribe = function() {
        service.unsubscribe("progress", cb);
    };
});